...
window.addEventListener('DOMContentLoaded', (event) => {
  ...
  function renderAllScores() {
    ...
    topUsers.forEach((item, index) => {
      ...
      if (validInput && item.score < MAX_SCORE) {
        const num = create('span', { textContent: `${index + 1}. `, style: 'color: #aaa; font-size: 1rem' });
        const scoreContent = `${unescape(nameStr.trim())}: ${unescape(scoreStr.trim())}`;
  ...
  $('#add-score-button').addEventListener('click', (event) => {
    ...
    validate(game.s).then((validationResolve, validationReject) => {
      ...
      const errorMessage = error === 'empty' ? 'Your name is required' : `Invalid name (${nameLength} letters)`;
  ...
  function validate(score) {
    return new Promise((resolve, reject) => {
      ...
      if (!playerNameValue.length || !score.currentMoves) {
        ...
        $('#player-name').focus();
        reject('empty');
      } else if (!validScore || !validPlayerName || playerNameValue.length > 20 || playerNameValue.length < 3) {
        reject('invalid');
      } else {
        resolve('success');
      }
